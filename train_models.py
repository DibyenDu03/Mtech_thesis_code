...
def _tower_loss(image_embeddings, labels,text_embeddings):
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(deploy_config.inputs_device()):
            ...
            import pickle 
            filename='/data/home/dibyendu//embedding/dataset/text-image_embedding.pickle'
            dbfile = open(filename, 'rb')      
            feature = pickle.load(dbfile) 
            dbfile.close() 
            ...
            for i in range(len(tar)):
                ...
                images.append(phot[i])
                texts.append(des[i])
                labels.append(tar[i])
            
            images=tf.convert_to_tensor(images, dtype=tf.float32)
            texts=tf.convert_to_tensor(texts, dtype=tf.float32)
            labels=tf.convert_to_tensor(labels, dtype=tf.float32)
            
            #### MY CODE END ####

            images, labels,texts = tf.train.batch(
                [images,labels,texts],
            ...
            images, labels, texts = batch_queue.dequeue()
                ...
                with tf.name_scope('tower_%d' % k) as scope:
                    with tf.variable_scope(tf.get_variable_scope()):
                        ...
                        loss, cmpm_loss, cmpc_loss, i2t_loss, t2i_loss = \
                            _tower_loss(images_splits[k], labels_splits[k],texts_splits[k])
